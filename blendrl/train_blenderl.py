...
class Args:
    ...
    mp_num_workers: int = 0
    """Seaquest multiprocessing workers; 0 selects a bounded automatic count"""
    mp_start_method: str = ""
    """optional multiprocessing start method; blank uses the backend default"""


def resolve_mp_worker_count(num_envs: int, requested_workers: int) -> int:
    if requested_workers > 0:
        return min(requested_workers, num_envs)
    cpu_count = os.cpu_count() or 1
    return max(1, min(num_envs, 16, cpu_count))


def build_vectorized_env(args: Args, mp_workers: int):
    if args.env_name == "seaquest":
        from blendrl.runtime.seaquest_mp_env import MultiprocessSeaquestEnv

        return MultiprocessSeaquestEnv(
            mode=args.algorithm,
            n_envs=args.num_envs,
            seed=args.seed,
            num_workers=mp_workers,
            start_method=args.mp_start_method or None,
        )
    return VectorizedNudgeBaseEnv.from_name(
        args.env_name, n_envs=args.num_envs, mode=args.algorithm, seed=args.seed
    )


def to_device_tensor(value, device: torch.device) -> torch.Tensor:
...
def main():
    ...
    resolved_mp_workers = resolve_mp_worker_count(args.num_envs, args.mp_num_workers)
    model_description = "{}_blender_{}".format(args.blend_function, args.blender_mode)
    backend_description = (
        f"mp_workers_{resolved_mp_workers}_" if args.env_name == "seaquest" else ""
    )
    learning_description = f"lr_{args.learning_rate}_llr_{args.logic_learning_rate}_blr_{args.blender_learning_rate}_gamma_{args.gamma}_bentcoef_{args.blend_ent_coef}_numenvs_{args.num_envs}_steps_{args.num_steps}_{backend_description}"
    ...
    envs = build_vectorized_env(args, resolved_mp_workers)
    if args.env_name == "seaquest":
        print(
            f"Using Seaquest multiprocessing backend with {resolved_mp_workers} worker processes"
        )
    ...
    next_logic_obs = to_device_tensor(next_logic_obs, device)
    next_obs = to_device_tensor(next_obs, device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            reward_tensor = to_device_tensor(reward, device).view(-1)
            rewards[step] = reward_tensor
            next_obs, next_logic_obs, next_done = (
                to_device_tensor(next_obs, device),
                to_device_tensor(next_logic_obs, device),
                to_device_tensor(next_done, device),
            )

            episodic_game_returns += reward_tensor
