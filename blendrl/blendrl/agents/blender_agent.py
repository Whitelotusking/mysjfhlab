...
class BlenderActor(nn.Module):
    ...
    def to_neural_action_distribution(self, neural_state):
        ...
        Returns:
            action_dist: action distribution
        """
        # CNNActor normalizes image observations in its public forward methods.
        # This path reaches the feature extractor directly, so it must apply the
        # same normalization before calculating the neural action distribution.
